...
This representation enables the application of linear algebraic operations across heterogeneous data types. The choice of this convention is not arbitrary—it aligns with the mathematical structure of linear transformations and optimization procedures that form the algorithmic backbone of machine learning. It is worth noting that some treatements will represent data as the transpose of $\mathbf{X}$; all of these results shown here are invariant to this choice of convention.
...
Since $\sum_{i=1}^n x_i = k$ and $\sum_{i=1}^n (1-x_i) = n - k$:
